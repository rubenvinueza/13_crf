...
Supervised: means you give the answer to the model. You provide the y variable to the model. (three machine learning models we learned)
Unsupervised: not give the y variable (k- means)
...
## Pros vs. cons of RF  
Pros:  
  - Good performance even without tuning  (individual trees have high variance but you average them so at the end you reduced the variance)
  - Simple to train  
...
> We used 10-fold CV (crossvalidation) before. It took about 10-min to run the grid on my side, so to avoid a long wait time in class, let's switch to 5-fold CV this time around.
...
resampling_foldcv <- vfold_cv(weather_train, 
                              v = 5) #do at leat 10 folds of data for class we did 5
...
rf_grid_result <- tune_sim_anneal(object = rf_spec,
                    ...
                    #param_info = rf_param,
                    iter = 10
                     ) # iterrations (iter) should be 100
